...
const Message=()=>{
    ...
    useEffect(() => {
        if (stompClient && auth.user && currentChat){
            console.log("yeah its coming inside");
            const subscription = stompClient.subscribe(`/user/${currentChat.id}/private`, onMessageReceive);
        }
    }, []);
    const sendMessageToServer=(newMessage)=>{
        if (stompClient && newMessage){
            stompClient.send(`/app/chats/${currentChat?.id.toString()}`, {}, JSON.stringify(newMessage));
        }
    }
                    ...
                    <div className="flex h-full justify-between space-x-2">
                        <div className="w-full">
                            <div className="flex space-x-4 items-center py-5">
                                <WestIcon/>
                                <h1 className="text-xl items-center font-bold">Home</h1>
                            </div>
                            <div className="h-[83vh] text-left">
                                <div className="">
                                    <SearchUser/>
                                </div>
                        ...
                        <div className="sticky bottom-0 border-l">
                            ...
                            <div className="py-5 flex items-center justify-center space-x-5">
                                <input onKeyPress={(e)=>{
                                    ...
                                    className="bg-transparent border border-[#3b40545] rounded-full w-[90%] py-3 px-5 "
                                    placeholder="Type Message..." type="text"/>
                                <div>
                                    <input
                                        type="file" accept="image/*" onChange={handleSelectImage} className="hidden"
                                           id="image-input"/>
